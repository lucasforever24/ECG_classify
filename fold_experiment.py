...
class EcgFoldExperiment(object):
    def __init__(self, config):
        ...
        self.weight = torch.Tensor([3, 1]).to(self.device)
        self.nll_loss = nn.NLLLoss(self.weight)

        pkl_dir = self.config.split_dir
        with open(os.path.join(pkl_dir, "folds.pkl"), 'rb') as f:
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(self.val_data_loader):
                ...
                if i % self.config.print_freq == 0:
                    print('Epoch: [{0}][{1}][{2}/{3}]\t'
                          'Loss: {loss.avg:.3f}\t'
                          'Prec@1 {total_accuracy.val:.3f} ({total_accuracy.avg:.3f})\t'.format(self.config.fold,
    ...
    def test(self):
        ...
        return total_accuracy, self.false_dict
