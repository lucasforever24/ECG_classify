...
def ExtractFeatures(high,name):
    ...
    for k in range(12):
        ...
        if k < 6:
            ...
            for i in range(Q_final , S_final):
                area[k] = area[k] + (High[i] - High[S_final])
    ...
    result[10] = T2[4]
    result[11] = T2[5]
    result[12] = abs(area[0])
    result[13] = abs(area[1])
    result[14] = abs(area[2])
    result[15] = abs(area[3])
    result[16] = abs(area[4])
    result[17] = abs(area[5])
    result[18] = Positive1 
    result[19] = Positive2
    result[20] = PvN
    result[21] = I_minus
...
train_dataset = datasets.ImageFolder(root='./Test_4_t/',transform=data_transform)
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(train_dataloader, 0):   
        ...
        #print(inputs.size())
        #print(labels.size())
        features = torch.FloatTensor(len(inputs[:,0,0,0]),30).zero_()
        target = torch.LongTensor(len(inputs[:,0,0,0])).zero_()
        curve = torch.FloatTensor(len(inputs[:,0,0,0]),12,240).zero_()
        for k in range(len(inputs[:,0,0,0])):    
            ...
            Positive = 0
            curve[k] = high
            features[k] = ExtractFeatures(high,name)
        if i == 0 :
            Features = features
            ...
            #print(Curve1.size())
            print(Features.size())
            print(str(k) + ' is completed')
        else:
            Features = torch.cat((Features,features))
            Curve = torch.cat((Curve,curve.unsqueeze(1)))
            Classification = torch.cat((Classification,target))
            print(str(k) +' is completed.')
            #print(Features)
            #print(Features1.size())
            #print(Classification)
    ...
    sio.savemat("Features_4_t.mat",{"Features":Features.numpy()})
    sio.savemat("Classification_4_t.mat",{"Classification":Classification.numpy()})
    sio.savemat("Curve_4_t.mat",{"Curve":Curve.numpy()})
    print("complete!")
