...
import argparse
from old_codes.network import CurveNetwork
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(train_dataloader, 0):   
        ...
        if i == 0 :
            ...
            Curve1 = torch.cat((Curve1,curve1.unsqueeze(1)))
    ...
    torch_curve1 = Data.TensorDataset(Curve1, Classification1)
        #print(Features.size())
    ...
    curveloader1 = DataLoader(
        dataset=torch_curve1,      # torch TensorDataset format
        batch_size=BATCH_SIZE,      # mini batch size
        shuffle=False,               # 要不要打乱数据 (打乱比较好)
        num_workers=2,              # 多线程来读数据
    )
    ...
    for i, data in enumerate(test_dataloader, 0):   
        ...
        if i == 0 :
            ...
            Curve2 = torch.cat((Curve2,curve2.unsqueeze(1)))
            Classification2 = torch.cat((Classification2,target2))
            #print(Features)
            #print(Features2.size())xewrttt
            #print(Classification)
            #print(Classification2.size())
            #print(Curve2.size())
    ...
    torch_curve2 = Data.TensorDataset(Curve2, Classification2)
        #print(Features.size())
        #print(Classification.size())
    loader2 = DataLoader(
    ...
    curveloader2 = DataLoader(
        dataset=torch_curve2,      # torch TensorDataset format
        batch_size=BATCH_SIZE,      # mini batch size
        shuffle=False,               # 要不要打乱数据 (打乱比较好)
        num_workers=2,              # 多线程来读数据
    )
    ...
    with open("acc(curvenet).txt", "w") as f:
        with open("log(curvenet).txt", "w")as f2:            
            for epoch in range(pre_epoch, EPOCH):
                ...
                for i, (data1 , data2) in enumerate(zip(curveloader1,loader1),0):    
                    # 准备数据
                    #print(i)
                ...
                with torch.no_grad():
                    correct = 0
                    total = 0
                    for i, ( data1 , data2) in enumerate(zip(curveloader2,loader2),0):   
                        model.eval() 
                        # 准备数据
                        #print(i)
                        curves, targets = data1
